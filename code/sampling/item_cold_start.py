# -*- coding: utf-8 -*-
"""
Created on Sat Sep 21 11:04:38 2019
...
## 平均每人推荐20部新番
def old_user_user_new_recall(user_id, user_item, W, K1=15, K2=20):
...
with open(r'C:\Users\tqd95\Desktop\graduation_thesis\dataset\sampling\old_new_cosine_dist_maxtrix.json','r', encoding='UTF-8') as f:
    W = json.load(f)
...
test_old_user_new_recall.to_pickle(r'C:\Users\tqd95\Desktop\graduation_thesis\dataset\sampling\test_old_user_new_recall.pkl')
...
u1 = set(user_item.user_id)
u2 = set(test_old_user_new_recall.user_id)
u3 = set(test.user_id)
# 问题：那些在u3里而不在u2里的id，是不是全是不在u1里的
len((u3-u2)&u1) # = 0
len(u3-u2)  # 168人，也是下面函数要处理的对象
...
# =============================================================================
# # 看看测试集中,所有观看了的记录中,新老番的比例：
# cond = test.start>'2017-06-30'
# test.loc[cond,'is_new'] = 1
# test.loc[-cond,'is_new'] = 0
# test.query('my_watched_episodes>0 and is_new == 1').groupby('user_id').size().describe()
# test.query('my_watched_episodes>0').groupby('user_id').size().describe()
# =============================================================================
