# -*- coding: utf-8 -*-
"""
Created on Sun Sep 15 10:28:27 2019
利用item-CF进行用户召回
@author: tqd95
"""
import json
...
def user_recall(user_id, user_item, user_watched_item, W, K1, K2):
    ...
    for a_id in user_liked_animes:
        ...
        #        break
    top_K2_animes = [i for i, w in sorted(
        rec_result.items(), key=lambda x:x[1], reverse=True)[:K2]]
    return top_K2_animes


# 对测试集用户，生成召回表
def get_test_recall(test, user_item, user_watched_item, W, K1=10, K2=10):
    '''
    @test: 不加任何加工的测试集;
...
with open(r'C:\Users\tqd95\Desktop\graduation_thesis\dataset\sampling\similarity_maxtrix.json', 'r', encoding='UTF-8') as f:
    W = json.load(f)

test_recall = get_test_recall(test=test, user_item=user_item,
                              user_watched_item=user_watched_item, W=W, K1=10, K2=10)
...
test_recall.to_pickle(r'C:\Users\tqd95\Desktop\graduation_thesis\dataset\sampling\test_recall.pkl')


# =============================================================================
# train_user = set(train.user_id.unique())
# test_user = set(test.user_id.unique())
# print('测试集里有%d个用户在训练集里没有出现' % len(test_user-train_user))    # 123人是新用户
# print('测试集里有%d个用户的召回数据是空的' % sum(list(map(lambda x:x==[],testing.anime_recall))))   # 168人无召回数据
#
# cond = testing.anime_recall.apply(lambda x:x==[])
# kong_users = set(testing.loc[cond,'user_id'])
# new_users = test_user - train_user
# # 推荐召回结果是空，但是不是新用户(即在训练集里出现过的用户，但是他们没有一条记录label=1..):
# # 这部分用户一共有45人。
# res = kong_users - new_users
# =============================================================================
