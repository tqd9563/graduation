# -*- coding: utf-8 -*-
"""
Created on Tue Sep 17 12:23:35 2019
生成用户画像特征
@author: tqd95
"""
import numpy as np
...
t1.drop('a_anime_id', axis = 1, inplace = True)     # t1就是我们的训练集join上动漫画像的结果,其中不包含打分等于0的项(status=4除外)

# =============================================================================
# # u1就是我们的用户画像骨架, u1的user_id是t1中出现过的所有user_id
# u1 = pd.DataFrame(t1.user_id.unique()).rename(columns = {0:'user_id'})
# u1 = pd.merge(u1, user_clean, on='user_id') \
#         .loc[:,['user_id','gender','location','birth_date','join_date']]
# =============================================================================
...
# 还是用百分比吧。。
train_plan_to_watch = pd.read_pickle(r'C:\Users\tqd95\Desktop\graduation_thesis\dataset\sampling\train_plan_to_watch.pkl')
...
# =============================================================================
# def get_topK_user_planned(train, fields, K=3):
#     '''
#     @train：用户plan to watch的数据集
#     @fields：根据哪个动漫特征来进行统计, 是string类型, 统计前K名
#     '''
#     tmp = train.groupby('user_id') \
#             .apply(lambda df:df[fields].value_counts(ascending=False)[:K].to_frame().reset_index())
#     
#     col_name_1 = 'u_planned_top_1_' + fields
#     col_name_2 = 'u_planned_top_2_' + fields
#     col_name_3 = 'u_planned_top_3_' + fields
#     res = tmp.unstack()['index'] \
#             .iloc[:,:K] \
#             .reset_index() \
#             .rename(columns = {0:col_name_1,1:col_name_2,2:col_name_3})
#     #res.drop('level_1',axis=1,inplace=True)
#     #res.drop(fields,axis=1,inplace=True)
#     #col_name = 'u_planned_' + fields +'_top1'
#     #res.rename(columns = {'index':col_name},inplace=True)
#     return res
# 
# u1 = pd.merge(u1, get_topK_user_planned(train_plan_to_watch,'a_source',K=1), on = 'user_id', how = 'left')
# u1 = pd.merge(u1, get_topK_user_planned(train_plan_to_watch,'a_anime_type',K=1), on = 'user_id', how = 'left')
# u1 = pd.merge(u1, get_topK_user_planned(train_plan_to_watch,'a_studio',K=1), on = 'user_id', how = 'left')
# 
# =============================================================================
...
u1.to_pickle(r'C:\Users\tqd95\Desktop\graduation_thesis\dataset\sampling\user_stats.pkl')
